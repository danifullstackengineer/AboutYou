...
const MenuPhone = ({
  ...
  return (
    <div className={`menuPhone ${chosenMode === false ? "menuPhone-dark" : "menuPhone-light"} ${clickedMenu ? "menuPhone-active" : "menuPhone-inactive"} ${mContext.isMobile ? "menuPhone-mobile" : ""} ${clickedBasket || clickedUser || clickedLanguage || clickedWishlist ? "menuPhone-clicked" : ""}`} ref={mainRef}>
      <div className={`menuPhone__top`}>
        <button className={clickedBasket ? "menuPhone__top-active" : ""} onClick={() => handleOpening("basket")}>Basket</button>
        <button className={clickedWishlist ? "menuPhone__top-active" : ""} onClick={()=> handleOpening("wishlist")}>Wishlist</button>
        <button className={clickedUser ? "menuPhone__top-active" : ""} onClick={() => handleOpening("user")}>Account</button>
        <button className={clickedLanguage ? "menuPhone__top-active" : ""} onClick={() => handleOpening("language")}>Language</button>
                ...
                <div className="menuPhone__bottom__option-option-bottom">
                  ...
                  <div className="menuPhone__bottom__option-option-right">
                    ...
                    <div className="menuPhone__bottom__option-option-right-btns">
                      <button type="button" onClick={()=> bContext.addToBasket(product)}>+</button>
                      <button type="button" onClick={() => bContext.decrementProduct(product.id)}>-</button>
                      </div>
                      <div className="menuPhone__bottom__option-option-right-btn-remove">
                          <button type="button" onClick={() => bContext.removeFromBasket(product.id)}>Remove</button>
                        </div>
                  </div>
                  </div>
                </div>
          ...
          wContext.product.map((product ,i) =>{
            return (
              <div className="menuPhone__bottom__option-option" key={i}>
                ...
                <div className="menuPhone__bottom__option-option-bottom">
                    <img alt="" src={product.backgroundImg + "1.jpg"}/>
                  <div className="menuPhone__bottom__option-option-right" style={{opacity: 0, visibility: "hidden"}}>
                    <span>
                      Quantity: {product.quantity}
                    </span>
                    <span>
                    ...
                    <div className="menuPhone__bottom__option-option-right-btns" style={{opacity: 0, visibility:"hidden"}}>
                      <button type="button" onClick={()=> bContext.addToBasket(product)}>+</button>
                      <button type="button" onClick={() => bContext.decrementProduct(product.id)}>-</button>
                      </div>
                      <div className="menuPhone__bottom__option-option-right-btn-remove">
                          <button type="button" onClick={() => wContext.removeFromWishlist(product.id)}>Remove</button>
                        </div>
                  </div>
                  </div>
                </div>
